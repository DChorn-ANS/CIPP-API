...
try {
    ...
    @{l = 'Name'; e = { if ($_.name -like "Standard Preset Security Policy*") { "Standard Preset Security Policy" }elseif ($_.name -like "Strict Preset Security Policy*") { "Strict Preset Security Policy" } else { $_.name } } } | Where-Object -FilterScript { $_.name -in $RuleState.name -or $_.IsDefault -eq $true -or $_.IsBuiltInProtection -eq $true }
