...
"@
          ...
          Write-LogMessage -API 'Alerts' -message "Sending alerts to $($Config.email) for $($tenant): $($_.Exception.message)" -sev info
        }
      }
    }
    else {
...
"@
        New-GraphPostRequest -uri 'https://graph.microsoft.com/v1.0/me/sendMail' -tenantid $env:TenantID -type POST -body ($JSONBody)
        Write-LogMessage -API 'Alerts' -message "Sending alerts to $($Config.email): $($_.Exception.message)" -sev info
      }
    }
  }
  catch {
