...
try {
  if ($Config.email -like '*@*' -and $null -ne $CurrentLog) {
    $JSONRecipients = $Config.email.split(",").trim() | % { '{"EmailAddress": {"Address": "' + $_ + '"}},' }
    $JSONRecipients = ([string]$JSONRecipients).Substring(0, ([string]$JSONRecipients).Length - 1)

    if ($config.onePerAlert) {
      foreach ($tenant in ($CurrentLog.Tenant | Sort-Object -Unique)) {
        foreach ($alert in ($CurrentLog | Where-object -Property tenant -EQ $tenant)) {
          $HTMLLog = ($alert | Select-Object Message, API, Tenant, Username, Severity | ConvertTo-Html -frag) -replace '<table>', '<table class=blueTable>' | Out-String
          $JSONBody = @"
                    {
                        "message": {
                          "subject": "CIPP Alert: Alert $($alert.message) for $tenant starting at $((Get-Date).AddMinutes(-15))",
                          "body": {
                            "contentType": "HTML",
