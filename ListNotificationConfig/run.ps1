...
$config.logsToInclude = ([pscustomobject]$config | Select-Object * -ExcludeProperty schedule, type, tenantid, alerting, onepertenant, oneperalert, seperateAlerts, partitionkey, rowkey, tenant, ETag, email, adminemail, logsToInclude, timestamp, webhook).psobject.properties.name
