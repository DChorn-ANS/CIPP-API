...
$config.logsToInclude = @(([pscustomobject]$config | Select-Object * -ExcludeProperty schedule, type, tenantid, seperateAlertTypes, alerting, onepertenant, oneperalert, partitionkey, rowkey, tenant, ETag, email, adminemail, logsToInclude, timestamp, webhook).psobject.properties.name)
