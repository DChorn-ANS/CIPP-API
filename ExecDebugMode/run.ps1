...
if (null -ne $request.body.setDebugMode) {
    Try {
        ...
        Update-AZFunctionAppSetting -Name $($ENV:WEBSITE_SITE_NAME) -ResourceGroupName $($ENV:Website_Resource_Group) -SubscriptionId $Subscription -AppSetting @{"DebugMode" = "$($request.body.setDebugMode)" } -Force
